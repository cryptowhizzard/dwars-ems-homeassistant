...
class ApiClient:
    ...
    def supervisor(
        ...
        *,
        ...
        result = self._request(self.supervisor_url, method, path, data, timeout=timeout)
        if isinstance(result, dict) and result.get("result") == "error":
            raise ApiError(
                f"{method} {path}: {result.get('message', 'Supervisor error')}",
    ...
    def wait_ha(self, timeout: int, interval: int = 10) -> bool:
        ...
        while time.monotonic() < deadline:
            try:
                if core_config_ready(self.ha("GET", "/config", timeout=20)):
                    return True
                time.sleep(interval)
            except ApiError:
                time.sleep(interval)
...
class AutoUpdater:
    def __init__(
        ...
        self._startup_core_check_pending = True
        self.core_recovery = CoreRecovery(
            options, client, self.state_store.path.with_name("dwars_core_recovery_state.json"), log
        )
    ...
    def ensure_core_available(self, context: str, timeout: int | None = None) -> tuple[bool, str]:
        if timeout is None:
            timeout = self.options.integer(
                "auto_full_system_update_core_recovery_timeout_sec", 900, minimum=120, maximum=7200
            )
        self.core_recovery.options = self.options
        ok, detail = self.core_recovery.ensure(context, timeout)
        self.state["core_health"] = dict(self.core_recovery.state)
        self.save()
        return ok, detail

    def restart_core_and_wait(self) -> tuple[bool, str]:
        # A stopped Core needs start, not a blind restart. Arm before issuing
        # any mutation so the daemon can recover after an interrupted call.
        self.core_recovery.enforce_boot_policy()
        self.core_recovery.arm("Core-herstart na componentupdate")
        if self.core_recovery.healthy():
            try:
                self.client.supervisor("POST", "/core/restart", {}, timeout=60)
            except ApiError as exc:
                if not api_error_may_be_interrupted_update(exc):
                    # Still check health: failure of a restart call is not
                    # evidence that Core is stopped (e.g. migration lock).
                    self.ensure_core_available("controle na geweigerde herstart")
                    return False, compact_exception(exc)
                log(f"Core restart-call werd onderbroken: {compact_exception(exc)}")
        return self.ensure_core_available("controle na Core-herstart")
    ...
    def update_system_component(
        ...
        *,
        ...
        if not to_bool(before.get("update_available"), False):
            if wait_for_ha:
                ok, detail = self.ensure_core_available("Core-versie al actueel: controleer ook beschikbaarheid")
        ...
        if wait_for_ha:
            self.core_recovery.arm("Core-update")
        try:
        ...
        if wait_for_ha:
            ready, detail = self.ensure_core_available("na Core-update")
            if not ready:
    ...
    def run_preflight_stage(self) -> None:
        self.core_recovery.enforce_boot_policy()
        ready, detail = self.ensure_core_available("voorcontrole systeemupdate")
        if not ready:
            self.add_failure("core_availability", detail)
            self.set_stage("verify", "Core niet beschikbaar; geen nieuwe updateacties starten.")
            return
    ...
    def run_core_stage(self) -> None:
        self.core_recovery.enforce_boot_policy()
        ready, detail = self.ensure_core_available("voor Core-check/update, ook na hostreboot")
        if not ready:
            self.add_failure("core_availability", detail)
            self.set_stage("verify", "Core niet beschikbaar; configuratiecheck/update uitgesteld.")
            return
    ...
    def run_verify_stage(self) -> None:
        ready, detail = self.ensure_core_available("eindcontrole systeemupdate")
        if not ready:
            self.add_failure("core_availability", detail)
    ...
    def run_once(self, reason: str = "scheduled") -> bool:
        try:
            with self.maintenance_lock(wait_seconds=10):
                self.state = self.state_store.load()
                self.core_recovery = CoreRecovery(
                    self.options, self.client,
                    self.state_store.path.with_name("dwars_core_recovery_state.json"), log
                )
    ...
    def recover_core_outside_run(self, *, startup: bool = False) -> None:
        startup = startup or self._startup_core_check_pending
        if not self.options.bool("auto_full_system_update", True):
            return
        try:
            with self.maintenance_lock(wait_seconds=1):
                self.state = self.state_store.load()
                if self.state.get("active"):
                    return  # active stage machine owns recovery, especially OS reboot
                self.core_recovery = CoreRecovery(
                    self.options, self.client,
                    self.state_store.path.with_name("dwars_core_recovery_state.json"), log
                )
                if startup:
                    self.core_recovery.enforce_boot_policy()
                    self.ensure_core_available("start DWARS updater / herstel vorige run")
                    self._startup_core_check_pending = False
                elif self.core_recovery.state.get("pending"):
                    self.ensure_core_available("hervat eerder Core-herstel", timeout=0)
                else:
                    return  # do not turn this into a permanent stop-overriding watchdog
                self.publish_status()
        except TimeoutError:
            pass  # installer holds lock; retry on the next scheduler tick
        except ApiError as exc:
            log(f"Core-herstel uitgesteld: {compact_exception(exc)}")

    def daemon(self) -> None:
        self.recover_core_outside_run(startup=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument("--ensure-core", action="store_true", help="only check/start Core, do not update")
    group.add_argument("--restart-core", action="store_true", help="restart/start Core after component changes")
    parser.add_argument("--lock-held", action="store_true", help="internal: caller already holds maintenance lock")
...
def main() -> int:
    ...
    if args.ensure_core or args.restart_core:
        ...
        try:
            with lock:
                updater.core_recovery.enforce_boot_policy()
                ok, detail = (
                    updater.restart_core_and_wait() if args.restart_core
                    else updater.ensure_core_available("gerichte Core-controle")
                )
                log(detail)
                return 0 if ok else 1
        except TimeoutError:
            log("Core-actie uitgesteld: onderhoudslock bezet.")
            return 75
    if args.run_now:
