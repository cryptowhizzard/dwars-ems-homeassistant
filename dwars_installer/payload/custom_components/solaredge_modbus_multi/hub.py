...
class SolarEdgeModbusMultiHub:
    def __init__(
        ...
        self._mac_address = normalize_mac(entry_data.get(CONF_MAC))
    ...
    async def async_record_mac_if_missing(self) -> bool:
        """Record the network MAC address for DHCP/IP recovery when possible."""
        if self._mac_address:
            return False

        mac_address = await async_get_mac_from_host(self._host, self._port)
        if not mac_address:
            return False
        ...
        if config_entry is not None:
            self._hass.config_entries.async_update_entry(
                config_entry,
                unique_id=f"{mac_address}:{self._port}",
                data={**config_entry.data, CONF_MAC: mac_address},
            )
        _LOGGER.info(
            "Recorded SolarEdge network MAC %s for %s:%s",
            mac_address,
            self._host,
            self._port,
        )
    ...
    async def async_recover_host_by_mac(self) -> bool:
        """Find and switch to a new IP address when DHCP changed it."""
        if not self._mac_address:
            return False

        new_host = await async_find_host_for_mac(
            self._hass,
            self._mac_address,
            port=self._port,
        )
        if not new_host or new_host == self._host:
            return False
        ...
        if config_entry is not None:
            self._hass.config_entries.async_update_entry(
                config_entry,
                unique_id=f"{self._mac_address}:{self._port}",
                data={
                    **config_entry.data,
                    CONF_HOST: new_host,
        ...
        _LOGGER.warning(
            "SolarEdge inverter MAC %s moved from %s to %s; updated config entry",
            self._mac_address,
            old_host,
            new_host,
        )
    ...
    async def async_refresh_modbus_data(self) -> bool:
        ...
        if not self.is_connected:
            await self.connect()
            if not self.is_connected and await self.async_recover_host_by_mac():
                await self.connect()
        ...
        if not self.initalized:
            try:
                async with asyncio.timeout(self.coordinator_timeout):
                    ...
                    await self.async_record_mac_if_missing()
    ...
    def mac_address(self) -> str | None:
        """Return the stored network MAC address."""
        return self._mac_address

    @property
    def primary_inverter_unit_id(self) -> int | None:
        """Return the first configured inverter unit ID for the network MAC connection."""
        try:
            return min(int(unit_id) for unit_id in self._inverter_list)
...
class SolarEdgeInverter:
    ...
    def device_info(self) -> DeviceInfo:
        ...
        if (
            self.hub.mac_address
            and self.hub.primary_inverter_unit_id == self.inverter_unit_id
        ):
            device_info["connections"] = {
                (dr.CONNECTION_NETWORK_MAC, self.hub.mac_address)
            }
        return device_info
