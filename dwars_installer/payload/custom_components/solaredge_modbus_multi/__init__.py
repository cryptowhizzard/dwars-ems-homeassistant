...
from .const import DEFAULT_AREA, DOMAIN, ConfDefaultInt, ConfName, RetrySettings
...
@callback
def _async_assign_default_area(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Put newly created SolarEdge devices in the default area if no area is set."""
    area_registry = ar.async_get(hass)
    area = area_registry.async_get_area_by_name(DEFAULT_AREA)
    if area is None:
        area = area_registry.async_create(DEFAULT_AREA)
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _async_assign_default_area(hass, entry)
...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if config_entry.version == 2 and config_entry.minor_version < 2:
        _LOGGER.debug("Migrating from version 2.1 to 2.2")
        hass.config_entries.async_update_entry(
            config_entry,
            version=2,
            minor_version=2,
