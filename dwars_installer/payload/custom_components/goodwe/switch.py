...
async def async_setup_entry(
    ...
    auto_load_control = config_entry.options.get(
        CONF_AUTO_LOAD_CONTROL,
        config_entry.data.get(CONF_AUTO_LOAD_CONTROL, DEFAULT_AUTO_LOAD_CONTROL),
    )
    ...
    for description in SWITCHES:
        try:
            ...
            if (
                description.key == "load_control"
                and auto_load_control
                and current_state != 1
            ):
                try:
                    await inverter.write_setting(description.setting, 1)
                    current_state = 1
                    _LOGGER.info("Enabled GoodWe load control by default")
                except (InverterError, ValueError) as err:
                    _LOGGER.warning(
                        "Could not enable GoodWe load control by default: %s", err
                    )
...
class InverterSwitchEntity(
    ...
    """Switch representation of inverter settings."""
