...
from .const import CONF_MAC, DOMAIN, ConfDefaultStr, ConfName
from .helpers import device_list_from_string, host_valid
from .network_discovery import async_get_mac_from_host, normalize_mac
        ...
        if user_input is not None:
            ...
            try:
                ...
                if not host_valid(user_input[CONF_HOST]):
                    ...
                    mac_address = normalize_mac(self._entry.data.get(CONF_MAC))
                    if not mac_address:
                        mac_address = await async_get_mac_from_host(
                            user_input[CONF_HOST], user_input[CONF_PORT]
                        )

                    if mac_address:
                        user_input[CONF_MAC] = mac_address
                        this_unique_id = f"{mac_address}:{user_input[CONF_PORT]}"
                    else:
